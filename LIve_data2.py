# ========================================
# FILE: menu_page1_live_dashboard_mqtt_debug_autorefresh.py
# Updated with auto-refresh every minute and IP-based MQTT
# ========================================
...
# Auto-Refresh Setup
# ----------------------------
AUTO_REFRESH_INTERVAL = 2  # Refresh every 60 seconds
...
if 'refresh_thread' not in st.session_state:
    refresh_thread = threading.Thread(target=force_refresh, daemon=True)
    refresh_thread.start()
    st.session_state.refresh_thread = refresh_thread

# ----------------------------
# MQTT Setup with IP Address
# ----------------------------
mqtt_broker = "134.209.100.187"  # Using IP address instead of domain
mqtt_port = 1883
topics = {
...
st.title("🚻 Toilet Hygiene Live Dashboard (MQTT with Auto-Refresh)")

# Refresh info
last_refresh = datetime.datetime.now().strftime("%H:%M:%S")
st.caption(f"🔄 Auto-refresh every {AUTO_REFRESH_INTERVAL} seconds | Last refresh: {last_refresh}")
